...
$(document).ready(function() {
	$('#logform').submit(async function() {
		...
		let response1 = await checksession('accessingDatabaseFile'); // start access for this AJAX
		let user = {
			login: $('#login1').val(),
		...
		let response2 = await checksession('closingDatabaseFile'); // close access for this AJAX
